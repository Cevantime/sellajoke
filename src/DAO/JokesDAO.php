...
class JokesDAO extends DAO
{
    public function buildEntity(array $datas)
    {
        ...
        foreach (['text', 'title', 'id'] as $field) {
        ...
        if (! empty($datas['category_name'])) {
             ...
             */
            $catDatas = [];
            foreach (['icon', 'name','id'] as $fieldCat) {
     ...
     * @param type $offset
     */
    public function findLast(array $where = [], $limit = null, $offset = null)
    {
        ...
        $datas = $this->db->fetchAll($query->getSQL(), empty($params) ? [] : $params);
    ...
    {
        $datas = [];
        foreach (['title', 'image'] as $basicField) {
        ...
        $this->db->update('jokes', $data, ['id'=>$joke->getId()]);
    }
